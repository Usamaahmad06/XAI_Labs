...
class SR_DQN(OffPolicyAlgorithm):
    ...
    def _get_suggested_action(self, observed_img) -> List[int]:
        ...
        keys_f = set() 
        key_in_front = False
        observed_img = np.asarray(observed_img)
        if observed_img.size == 7 * 7 * 3:
            observed_img = observed_img.reshape((7, 7, 3))
        else:
            observed_img = observed_img.reshape(-1)
            if observed_img.size >= 7 * 7 * 3:
                observed_img = observed_img[: 7 * 7 * 3].reshape((7, 7, 3))
            else:
                observed_img = observed_img[0].reshape((7, 7, 3))
        for i in range(7):
            for j in range(7):
    ...
    def _get_random_action(self, observation):
        suggested = self._get_suggested_action(observation)
        n_actions = self.action_space.n

        if not suggested:
            return int(self.action_space.sample())

        weights = np.zeros(n_actions, dtype=np.float64)
        non_suggested = [a for a in range(n_actions) if a not in suggested]

        for action in suggested:
            weights[action] = self.conf_level / len(suggested)
        if non_suggested:
            share = (1.0 - self.conf_level) / len(non_suggested)
            for action in non_suggested:
                weights[action] = share

        weights /= weights.sum()
        return int(np.random.choice(n_actions, p=weights))
