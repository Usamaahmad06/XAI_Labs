...
class Pendulum(gym.Env):
    ...
    * v0: Initial versions release
    ...
    def step(self, u):
        ...
        tau_state = [list(pair) for pair in self.past_tau_trajectory]
        tau_state.append([float(thdot), torque])
        tau_state = tau_state[-self.tau :]

        _, single_rho = self.parser.compute_robustness_dense(tau_state)
        rho_thetadot = float(single_rho["thetadot_constraint"])
        rho_torque = float(single_rho["torque_constraint"])

        cost_thetadot = float(np.tanh(-self.beta * rho_thetadot))
        cost_torque = float(np.tanh(-self.beta * rho_torque))

        newthdot = thdot + (
            -3 * g / (2 * l) * np.sin(th + np.pi) + 3.0 / (m * l**2) * torque
        ) * dt
        ...
        reward = -(th_normalized**2 + 0.1 * thdot**2 + 0.001 * (torque**2))
