...
class QNetwork(nn.Module):
    ...
    def get_action(self, x, stored_rule_actions=None, action=None, skip=False, epsilon=1.0):
        ...
        rule_factor = 1 + epsilon * self.rule_pmf[0, 0]
        combined_pmfs = torch.where(
            rule_mask.unsqueeze(-1),
            combined_pmfs * rule_factor,
            combined_pmfs,
        )
...
def h_c51():
    ...
    for global_step in progress_bar:
        ...
        if "final_info" in infos:
            ...
            "episode" in infos
            and infos["episode"].get("_r") is not None
            and infos["episode"]["_r"][0]
        ):
            episodes_returns.append(float(infos["episode"]["r"][0]))
            episodes_lengths.append(float(infos["episode"]["l"][0]))
            episode_done = True

        if episode_done and global_step >= print_step:
        ...
        for idx, trunc in enumerate(truncations):
            if trunc and infos.get("final_observation") is not None:
