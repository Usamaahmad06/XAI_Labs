...
def learn_causal_model(normal_csv_path: str, save_path: str):
    ...
    pcmci = PCMCI(dataframe=pp.DataFrame(nonconst_data), cond_ind_test=ParCorr())
...
def fit_normal_coeffs(normal_data: np.ndarray, causal_matrix: np.ndarray):
    ...
    for var in np.unique(indices[1, :]):
        ...
        var_indices.sort(key=lambda a: a[2])
        max_delay = var_indices[-1][2]
        stack_list = []
        for el in var_indices:
            stack_list.append(normal_data[max_delay - el[2]: normal_data.shape[0] - el[2], el[0]])
        stack_list.append(np.ones(normal_data.shape[0] - max_delay))
        coeffs = np.linalg.lstsq(
            np.column_stack(stack_list),
            normal_data[max_delay:, var],
            rcond=None,
        )[0][:-1]
...
# ================================================================
def plot_feature_importance_subplots(norm_agg_list, indices, nonconst, var_names, attack_names, top_frac=0.1, save_path=None):
    """
...
def _require_pepper_data():
    required = [
        os.path.join(PREFIX, "normal.csv"),
    ...
    if missing:
        raise FileNotFoundError(
            "Pepper dataset not found. Download CSV files from "
            "https://sites.google.com/diag.uniroma1.it/robsec-data "
            f"and place them in: {PREFIX}\n"
            "Required files:\n  - " + "\n  - ".join(os.path.basename(p) for p in required)
        )
...
def main(output_dir=None):
    ...
    results = {
        ...
        "recall": float(recall),
        "f1": float(f1),
        "true_positives": [int(x) for x in tpos],
        "false_positives": [int(x) for x in fpos],
        "false_negatives": [int(x) for x in fneg],
    ...
    if output_dir:
        ...
        with open(os.path.join(output_dir, "metrics.json"), "w", encoding="utf-8") as f_out:
            json.dump(results, f_out, indent=2)

    plot_save = os.path.join(output_dir, "feature_importance.png") if output_dir else None
    plot_feature_importance_subplots(
        norm_agg_attacks, indices, nonconst, f["var"], attack_names,
        top_frac=0.1, save_path=plot_save,
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Causal anomaly detection lab (Pepper robot)")
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="If set, save plots and metrics.json to this folder.",
    )
