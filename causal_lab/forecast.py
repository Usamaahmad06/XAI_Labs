...
def fit_causal_model(df, target_col=TARGET, tau_max=MAX_TAU):
    ...
    train_indices = list(range(n_train - tau_max))
    test_indices = list(range(n_train, T))
...
# -------------------------- PLOTTING --------------------------
def plot_predictions(true, pred_causal, pred_tcn, nmae_causal, nstd_causal, nmae_tcn, nstd_tcn, save_path=None):
...
def plot_feature_importance(causal_feat_names, causal_importance, tcn_feat_names, tcn_importance, top_k=12, save_path=None):
    """
...
def main(output_dir=None):
    ...
    pred_save = feat_save = None
    if output_dir:
        os.makedirs(output_dir, exist_ok=True)
        pred_save = os.path.join(output_dir, "predictions.png")
        feat_save = os.path.join(output_dir, "feature_importance.png")

        causal_predictor_list = [
            {"variable": df_b1.columns[var_idx], "lag": int(lag)}
            for var_idx, lag in predictors[target_idx]
        ]
        results = {
            "target": TARGET,
            "tau_max": int(tau_max),
            "n_causal_predictors": len(predictors[target_idx]),
            "n_tcn_features": len(tcn_feat_names),
            "nmae_causal": float(nmae_causal),
            "nstd_causal": float(nstd_causal),
            "nmae_tcn": float(nmae_tcn),
            "nstd_tcn": float(nstd_tcn),
            "causal_predictors": causal_predictor_list,
            "causal_feature_importance": {
                name: float(val) for name, val in zip(causal_feat_names, causal_importance)
            },
        }
    ...
    plot_predictions(
        true_causal, pred_causal, pred_tcn,
        nmae_causal, nstd_causal, nmae_tcn, nstd_tcn,
        save_path=pred_save,
    )
    plot_feature_importance(
        causal_feat_names, causal_importance, tcn_feat_names, tcn_importance,
        save_path=feat_save,
    )

    if output_dir:
        print(f"Results saved to: {output_dir}")
    return results if output_dir else None


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Causal forecasting lab (Building 1 cooling demand)")
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="If set, save plots and metrics.json to this folder instead of showing plots.",
    )
